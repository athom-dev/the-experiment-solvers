...
const getSolverMode = (cells: GridCell[]): SolverMode => {
  const hasRednOnes = cells.some((cell) => cell.value === 4) && cells.some((cell) => cell.value === 1) && cells.some((cell) => cell.value === 2)
    ...
    >
      ...
      <HStack align="stretch">
        {variationRenderers.map((item, index) => (
          <RadioCard.Item key={index} value={`${index}`} w="50px" h="50px">
  ...
  const grid = useMemo(
    () => (
      <Box w="400px" aspectRatio="1/1" position="relative">
        ...
        <GridGenerator.Root w="100%" h="100%" labelGrid bg="gray.200" gap="2px" p="3px" x={GRID_SIZE} y={GRID_SIZE}>
